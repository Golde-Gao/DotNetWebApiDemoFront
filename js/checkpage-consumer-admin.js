...
function LoadWholePage() {
    taskUserID = getQueryVariable("userid");
...
function InitPage() {
        ...
        $.ajax({ type: "GET", url: taskdetailurl, xhrFields: XHRCONF, })
            .then(catedata => {
                ...
                $.each(catedata.data, (_, eele) => {
                    if (eele.userid == taskUserID && eele.subjectid == _TaskData.subjectId) {
    ...
    //加载任务整体信息
    var taskdetailurl = apiconf.n_taskdetail + "?taskId=" + TASKID + "&userId=" + taskUserID;
    $.ajax({ type: "GET", url: taskdetailurl, xhrFields: XHRCONF, })
        .then(data => {
            _TaskData = data.data;
...
function EventRegister() {
    ...
    //任务审核确认事件
    $("#button-background-task-submit").click(_ => PostTaskCheck());
    ...
    $("#button-memo").click(_ => {
        layui.layer.open({
            ...
            success: _ => {
                ...
                var queryUrl = apiconf.n_memoGet + "?testEntityID=" + _RecAndCheckData.currentShiTiID + "&userID=" + taskUserID + "&taskID=" + TASKID;
    ...
    $("#button-chongxinshibie").click(event => {
        ...
        var taskdetailurl = apiconf.n_shitiRetestdetail + "?taskId=" + TASKID + "&userId=" + taskUserID + "&testEntityId=" + _RecAndCheckData.currentShiTiID;
        $.ajax({ type: "GET", url: taskdetailurl, xhrFields: XHRCONF, })
            .then(data => {
    ...
    $("#button-jiansuo-gaoji").click(event => {
                ...
                $.ajax({ type: "GET", url: taskdetailurl, xhrFields: XHRCONF, })
                    .then(data => {
                        ...
                        $.each(data.data, (idx, eee) => {
                            if (eee.userid == taskUserID && eee.subjectid == _TaskData.subjectId) {
    ...
    //添加漏识别考点类型弹框（已有的主题查询，根据赵庚的excel存在表里查出来）
    ...
    if (CurrentShiTiList != null && CurrentShiTiList.length > 0) {
        ...
        $.ajax({ type: "GET", url: taskdetailurl, xhrFields: XHRCONF, })
            .then(data => {
                ...
                var ratiourl = apiconf.n_taskAndUserRatio + "?&taskId=" + TASKID + "&userId=" + taskUserID;
                ...
                var shitiurltmp = apiconf.n_memoGet + "?testEntityID=" + _RecAndCheckData.currentShiTiID + "&userID=" + taskUserID + "&taskID=" + TASKID;
...
function LoadLeftSideCategory() {
    ...
    $.ajax({ type: "GET", url: apiconf.n_tagQuery, xhrFields: XHRCONF, })
        .then(data => {
            ...
            var queryUrl = apiconf.n_tagShitTiMogify + "?taskId=" + TASKID + "&userId=" + taskUserID + "&entityId=" + _RecAndCheckData.currentShiTiID;
        ...
        layui.mouseRightMenu.open(menu_data, cconf, function(event) {
            if (event.title == "删除") {
                $.ajax({
                    ...
                    contentType: "application/json",
                    xhrFields: XHRCONF,
                    data: '{"categoryname":"' + event.data.tegoryName + '","userid":' + taskUserID + ',"id":' + event.data.categoryID + '}'
                }).then(deleD => {
                    if (deleD.data == 0) {
