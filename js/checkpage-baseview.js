...
$(() => {
    ...
    for (i = 0; i < 113; i++) {
        ...
        if (i < 5) {
            if (i % 3 == 1) {
                tttt = '<span class="layui-badge-dot layui-bg-orange" style="margin-right:10px"></span>' + tttt + '<span style="display:inline-block;width:10px;margin-left:10px;"><i class="layui-icon layui-icon-ok" style="font-size: 30px; color: #1E9FFF;font-size:x-small"></i></span>';
            } else {
                tttt = '<span style="margin-right:18px"></span>' + tttt + '<span style="display:inline-block;width:10px;margin-left:10px;"><i class="layui-icon layui-icon-ok" style="font-size: 30px; color: #1E9FFF;font-size:x-small"></i></span>';
            }
        } else {
            if (i % 3 == 1) {
    ...
    $("#button-shitilist-show").hide();
    $("#button-shitilist-hide").css("display", "inline-block");
    let heiii = window.innerHeight * 0.5 - 77;
    leftlayindex = layer.open({
        id: "",
        type: 1,
        title: false,
        ...
        area: ['370px', heiii + 'px'],
        success: function() {
            $("#test-item-container").css("height", heiii - 80 + "px");
            $("#test-item-container").css("overflow-y", "auto");
        }
    });
    ...
    let ttitle = '<b style="margin-right:20px;">最新结果</b> 测试时间<span style="margin-left:10px;" class="text-show">2021-04-25 10:30:06</span><b style="margin-left:20px;">陈瑞（未审核）</b>';
    // OpenDuiBi(ttitle);
